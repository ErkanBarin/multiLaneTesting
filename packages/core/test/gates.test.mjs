// no-runtime-ai:allow — the fixtures below must contain the banned imports verbatim; that is what
// proves the gate catches them. Scanning this file would make it report its own test data.
...
test('no-runtime-ai gate exempts a marked file but reports it instead of hiding it', () => {
  const root = tmpProject();
  mkdirSync(join(root, 'tests'), { recursive: true });
  writeFileSync(join(root, 'tests', 'patterns.mjs'), "// no-runtime-ai:allow\nimport anthropic from 'anthropic';\n");
  writeFileSync(join(root, 'tests', 'real.mjs'), "import anthropic from 'anthropic';\n");

  const res = runNoRuntimeAiGate({ cwd: root });
  assert.equal(res.ok, false, 'an unmarked violation next to a marked file must still fail');
  assert.ok(res.violations.every((v) => /real\.mjs/.test(v.path)));
  // The exemption is visible in the result, never a silent skip.
  assert.ok(res.exempt.some((p) => /patterns\.mjs/.test(p)));
  assert.equal(res.scanned, 1, 'an exempt file is not counted as scanned');
});
