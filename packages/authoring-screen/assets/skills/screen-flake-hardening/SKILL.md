...
| Passes locally, fails in CI | Wrong partition, or live `PROD` data leaked in | Fix the **RPS scenario / `SCREEN_RPS_PARTITION`** |
...
5. Record the root cause in this project's blocker log or frozen-locator inventory.
...
- "Fix" a partition failure by retargeting the operational partition. A test partition
  (`TEST_A`/`TEST_B`/`TEST_C`) is the only legal replay target, **never `PROD`**; the runtime refusal is
