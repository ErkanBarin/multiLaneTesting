...
[`@erkanbarin/stomp`](../stomp/README.md), and never a dependency of it.

```
@erkanbarin/stomp            = STOMP runtime capability (subscribeOnce, supervised send)
@erkanbarin/authoring-stomp   = STOMP skill, agent and metadata (this package)
```
...
Zero dependencies, no executable test logic, and `@erkanbarin/stomp`'s `package.json` does not
